...
namespace QueryIt
{
    class Program
    {
        ...
        private static Manager ConvertIt(Person person)
        {
            return new Manager();
        }

        private static void AddManagers(IWriteOnlyRepository<Manager> employeeRepository) //Do mniej szczegółowych typów zostanie dodany bardziej szczegółowy
        {
            employeeRepository.Add(new Manager { Name = "Alex" });
            employeeRepository.Commit();
        }

        private static void DumpPeople(IReadOnlyRepository<Person> employeeRepository) //Bardziej szczegółowe typy zostaną zwrócone jako mniej szczegółowe
        {
